  ...
  return (
    <div className='flex flex-1 flex-col gap-6'>
      <p className='text-sm text-muted-foreground'>
        <Flame className='me-1.5 mb-0.5 inline size-4 text-orange-500' />
        Quản lý iptables DERP-FORCE chain trên từng node. Client IP trong danh
        sách sẽ được ACCEPT vào chain riêng — ép traffic qua DERP chỉ định.
      </p>
      ...
      <div className='rounded-md border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-muted-foreground'>
        ...
        <ul className='mt-1 list-inside list-disc space-y-0.5'>
          <li>
            ...
            <code className='font-mono'>~/.ssh/authorized_keys</code> trên từng
            DERP VPS.
          </li>
          <li>
            SSH user cần quyền chạy <code className='font-mono'>iptables</code>{' '}
            (root hoặc sudo NOPASSWD).
          </li>
          <li>
            Chain <code className='font-mono'>DERP-FORCE</code> chỉ ACCEPT —
            không DROP client ngoài danh sách. Để chặn, thay đổi default policy
            INPUT thành DROP trên VPS.
          </li>
          <li>
            <code className='font-mono'>Sync iptables</code> = áp dụng lại toàn
            bộ danh sách active. <code className='font-mono'>Clear chain</code>{' '}
            = xóa chain khỏi VPS.
          </li>
        </ul>
      </div>
