...
/**
 * Khai báo 1 VPN gateway (+ domain đi qua nó) từ env lúc khởi động (idempotent)
 * — để deploy dựng gateway + agent token + PAC declaratively, không cần gọi API.
 *
 ...
 *   KHÔNG ghi đè giá trị admin đã đặt / token đã xoay tay.
 * - VPN_GW_DOMAINS (CSV) -> thêm vào vpn_domains (bỏ qua nếu đã có).
 *
 * Trả true nếu vừa tạo mới gateway record (để log).
 */
export async function bootstrapVpnGateway(): Promise<boolean> {
  ...
  if (!existing) {
    ...
    if (Object.keys(set).length > 0) {
      ...
      await db.update(vpnGateways).set(set).where(eq(vpnGateways.id, gatewayId))
    }
  }

  // Seed domain đi qua gateway (idempotent) — để PAC route ngay.
  const domains = env.VPN_GW_DOMAINS.split(',').map((s) => s.trim()).filter(Boolean)
  for (const domain of domains) {
    const [dup] = await db
      .select({ id: vpnDomains.id })
      .from(vpnDomains)
      .where(and(eq(vpnDomains.gatewayId, gatewayId), eq(vpnDomains.domain, domain)))
      .limit(1)
    if (!dup) await db.insert(vpnDomains).values({ gatewayId, domain })
  }
